...
# this part has moved to "master" where env.var. CURRENTDATE is defined/exported
#
...
fcp -c /usr/bin/rcp ${TARFILE} ibtevnfsg4:/lfstev/g4p/yarba_j/g4studies/${CURRENTDATE}/analysis_${proc_level}_${beam}_ITEP
...
if [ ! -d "/lfstev/g4p/yarba_j/g4studies/${CURRENTDATE}/analysis_${proc_level}_${beam}_ITEP" ]; then
/bin/mkdir /lfstev/g4p/yarba_j/g4studies/${CURRENTDATE}/analysis_${proc_level}_${beam}_ITEP
fi
...
#momz_list=( 5.0 7.5 )
#if [ x"$beam" = x"piminus" ] || [ x"$beam" = x"piplus" ]; then
#momz_list=( 5.0 )
#fi

# could be a useful cross check in the future; see comment earlier
#ntgt=${#target_list[@]}
#nmomz=${#momz_list[@]}
#njobstotal=$(( $ntgt * $nmomz  ))
#if [ ${njobstotal} -ge ${coresPerNode} ]; then
#  echo "num. of target * num. of momenta = ${njobstotal"; exceeds num. of cores = ${coresPerNode} on ${nodeName}" 
#fi

for target in ${target_list[@]}; do
for momz in ${momz_list[@]}; do
echo " submitting ITEP analysis for ${beam} of ${momz}GeV on ${target} " 
