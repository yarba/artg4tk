...
#
# FIXME !!! NEED TO PASS IN ${DATE} !!!!
#           In principle, the whole path/to/${DATE} can be made an input/env.var.
#
evtfiledir=/lfstev/g4p/yarba_j/g4studies/${DATE}/${proc_level}_${beam}${momz}GeV_${target}
...
if [ ! -d "/lfstev/g4p/yarba_j/g4studies/${DATE}/analysis_${proc_level}_${target}" ]; then
/bin/mkdir /lfstev/g4p/yarba_j/g4studies/${DATE}/analysis_${proc_level}_${target}
fi

fcp -c /usr/bin/rcp ${TARFILE} ibtevnfsg4:/lfstev/g4p/yarba_j/g4studies/${DATE}/analysis_${proc_level}_${target}
...
#
beam_list=( proton=2212 piminus=-211 )
momz_list=( 5.0 )
...
for (( i=0; i<${#beam_list[@]}; ++i )) do
export beam=`echo ${beam_list[$i]} | awk -F '=' '{print $1}'`
export pdgcode=`echo ${beam_list[$i]} | awk -F '=' '{print $2}'`
